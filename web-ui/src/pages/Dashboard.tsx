      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.5fr 1fr', gap: '16px' }}>
        ...
        <div className="card" style={{ padding: '24px', display: 'flex', flexDirection: 'column' }}>
          ...
          {(activeRun || latestRun) && (() => {
            ...
            const verifications = latestRun?.verification || [];
            const confidence = latestRun?.confidence || 0;
              ...
              <>
                ...
                <div style={{ marginBottom: '20px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.78rem', color: 'var(--text-secondary)', marginBottom: 6 }}>
                    <span>{isActive ? '⚡ Running...' : `Status: ${latestRun?.status || '—'}`}</span>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 'auto', paddingTop: '16px', borderTop: '1px solid var(--surface-border)' }}>
                  {[
                    ...
                    { label: 'Files', value: latestRun?.files_changed != null ? String(latestRun.files_changed) : '—' },
                    { label: 'Tokens', value: latestRun?.tokens ? fmtTokens(latestRun.tokens) : '—' },
                    { label: 'Duration', value: latestRun?.duration || (isActive ? '...' : '—') },
                  ].map(m => (
                    <div key={m.label}>
        ...
        <div className="card" style={{ padding: '20px', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: '12px', flex: 1 }}>
            {SYSTEM_COMPONENTS.map((sys, i) => {
              ...
              return (
                <div key={i} style={{ padding: '16px 8px', background: '#f8fafc', border: '1px solid var(--surface-border)', borderRadius: '8px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '10px', textAlign: 'center' }}>
