...
export function TaskRun({ onBack, runId, liveEvents = [], repoPath = '', issue = '', comparisonGroupId }: TaskRunProps) {
  ...
  const [runData, setRunData] = useState<any>(null); // persisted evidence from API

  // Poll /api/runs/{runId} to get persisted evidence for completed runs
  useEffect(() => {
    const poll = async () => {
      try {
        ...
        if (data.evidence || data.trace) setRunData(data);
      } catch {}
    };
    poll();
  ...
  // Poll comparison run data if this is an A/B run
  useEffect(() => {
    if (!comparisonGroupId) return;
    const poll = async () => {
      try {
        const res = await fetch('http://localhost:8000/api/dashboard');
        if (!res.ok) return;
        const data = await res.json();
        const grouped = (data.recent_runs || []).filter(
          (r: any) => r.comparison_group_id === comparisonGroupId
        );
        if (grouped.length > 0) setComparisonRuns(grouped);
      } catch {}
    };
    poll();
  ...
  const tabs = ['Trace', 'Plan', 'Tests', 'Logs', 'Metrics', 'Evidence'];
  ...
  // Token count from Summary events OR persisted trace
  ...
  if (totalTokens === 0 && runData?.trace?.total_tokens) totalTokens = runData.trace.total_tokens;
  ...
  // Confidence score - from events OR persisted evidence
  const confidenceEvent = liveEvents.find(e => e.message.includes('Overall Confidence:'));
  let confidence: number | null = confidenceEvent
    ? parseFloat(confidenceEvent.message.match(/[\d.]+/)?.[0] || '0')
    : null;
  if (confidence === null && runData?.evidence?.confidence?.overall_score !== undefined) {
    confidence = runData.evidence.confidence.overall_score;
  }

  // Verification results - from events OR persisted evidence
  const verifyEvents = liveEvents.filter(e => e.event_type === 'VERIFICATION_RESULT');
  const persistedVerifications: any[] = runData?.evidence?.verification_results || [];
  const usePersistedVerify = verifyEvents.length === 0 && persistedVerifications.length > 0;

  let testsPassed = 0, testsFailed = 0, testsSkipped = 0, testTotal = 0;
  if (usePersistedVerify) {
    testsPassed = persistedVerifications.filter((v: any) => v.passed).length;
    testsFailed = persistedVerifications.filter((v: any) => !v.passed).length;
    testTotal = persistedVerifications.length;
  } else {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      {/* Tabs */}
      <div style={{ display: 'flex', gap: '20px', borderBottom: '1px solid var(--surface-border)', marginBottom: '16px', flexShrink: 0, overflowX: 'auto', position: 'relative', zIndex: 10 }}>
        {tabs.map(tab => (
      ...
      {/* Tab Content */}
      <div style={{ display: 'flex', flexDirection: 'column', minHeight: '500px', flex: '1 1 auto' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
              <div style={{ position: 'relative' }}>
                ...
                {liveEvents.map((event, i) => {
                  const isError = event.event_type === 'ERROR' || event.message?.includes('[Policy Engine] REJECTED');
                  const isLayerStart = event.event_type === 'LAYER_START';
                  const isRecovery = event.message?.includes('[Recovery Engine]');
                  const timeStr = new Date(event.timestamp).toLocaleTimeString();
                  if (event.event_type === 'DIFF') return null;
                  return (
                    <div key={i} style={{ display: 'flex', gap: '12px', marginBottom: '16px', position: 'relative', zIndex: 1 }}>
                      <div style={{
                        width: 22, height: 22, borderRadius: '50%', flexShrink: 0,
                        background: isError ? 'var(--color-error)' : isRecovery ? '#eab308' : isLayerStart ? 'var(--color-success)' : 'var(--color-primary)',
                        display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', marginTop: 2
                      }}>
                        {isError ? <AlertCircle size={12} /> : isRecovery ? <RotateCcw size={12} /> : <CheckCircle size={12} />}
                      </div>
                      <div style={{ flex: 1 }}>
        ...
        {activeTab === 'Files Changed' && (
          <div className="card" style={{ display: 'flex', flexDirection: 'column', background: '#1e293b', color: '#f8fafc' }}>
        ...
        {activeTab === 'Tests' && (
          <div className="card" style={{ padding: '24px' }}>
            ...
            <div style={{ marginTop: 24 }}>
              {(usePersistedVerify ? persistedVerifications : verifyEvents).map((item: any, i: number) => {
                const label = usePersistedVerify ? (item.check_name || `Check ${i+1}`) : (item.message || '').split(':')[0];
                const passed = usePersistedVerify ? item.passed : (item.message || '').includes('PASS');
                return (
                  <div key={i} style={{ padding: '10px 14px', marginBottom: 8, borderRadius: 8, background: passed ? 'rgba(16,185,129,0.08)' : 'rgba(239,68,68,0.08)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: `1px solid ${passed ? 'rgba(16,185,129,0.2)' : 'rgba(239,68,68,0.2)'}` }}>
                    <span style={{ fontSize: '0.85rem' }}>{label}</span>
                    <span style={{ fontSize: '0.75rem', fontWeight: 600, color: passed ? '#10b981' : '#ef4444' }}>{passed ? 'PASS' : 'FAIL'}</span>
                  </div>
                );
              })}
            </div>
        ...
        {/* LOGS TAB */}
        {activeTab === 'Logs' && (
          <div className="card" style={{ display: 'flex', flexDirection: 'column', background: '#0f172a', color: '#e2e8f0' }}>
        ...
        {activeTab === 'Evidence' && (
          <div className="card" style={{ padding: '24px' }}>
            ...
            {runData?.evidence?.confidence?.factors && (
              <div style={{ marginTop: 24 }}>
                <h4 style={{ marginBottom: 12, fontWeight: 600, fontSize: '0.9rem', color: '#8b5cf6', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
                  Explain Run: Confidence Score Breakdown
                </h4>
                <div style={{ padding: '16px', background: 'var(--surface-color)', borderRadius: '8px', border: '1px solid var(--surface-border)' }}>
                  {Object.entries(runData.evidence.confidence.factors).map(([factor, score]: [string, any], i, arr) => (
                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: i === arr.length - 1 ? 'none' : '1px solid var(--surface-border)', fontSize: '0.875rem' }}>
                      <span style={{ textTransform: 'capitalize' }}>{factor.replace('_', ' ')}</span>
                      <span style={{ fontWeight: 600, color: (score as number) > 0 ? '#10b981' : '#ef4444' }}>
                        {(score as number) > 0 ? `+${((score as number) * 100).toFixed(0)}%` : '0%'}
                      </span>
                    </div>
                  ))}
            ...
            {(verifyEvents.length > 0 || persistedVerifications.length > 0) && (
              <div style={{ marginTop: 24 }}>
                ...
                {(usePersistedVerify ? persistedVerifications : verifyEvents).map((item: any, i: number) => {
                  ...
                  return (
                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 12px', borderBottom: '1px solid var(--surface-border)', fontSize: '0.875rem' }}>
                      <span>{label}</span>
                      <span style={{ color: passed ? '#10b981' : '#ef4444', fontWeight: 600 }}>{passed ? '✓ PASS' : '✗ FAIL'}</span>
                    </div>
                  );
                })}
              </div>
      ...
      {/* A/B Comparison - only shown for comparison runs */}
      {comparisonGroupId && comparisonRuns.length > 0 && (
