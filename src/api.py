...
async def start_run(req: RunRequest):
    ...
    run_registry[run_id] = {"issue": req.issue, "repo": req.repo_path.rstrip('/').split('/')[-1]}
    ...
    def background_task():
        try:
            ...
            orchestrator = Orchestrator(repo_path=repo_path, issue_statement=req.issue, logger=logger, run_id=run_id)
            orchestrator.run()
